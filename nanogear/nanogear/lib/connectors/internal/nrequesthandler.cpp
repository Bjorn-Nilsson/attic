...
NRequestHandler::NRequestHandler(QTcpSocket *client) : m_clientSocket(*client) {}

void NRequestHandler::run() {
  exec();
}

void NRequestHandler::readyRead()
{
  if (m_clientSocket->waitForReadyRead()) {
    ...
    // The ',' in the template declaration is interpreted as an
    // argument separator
    ...
    if (entity.hasFormat("application/x-www-form-urlencoded")) {
      //             ↓ workaround to make QUrl recognize a query string
